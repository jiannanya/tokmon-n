...
`models.default` 必须指向 `models.goes` 中存在且启用的配置名称。协议可为 `local`、`openai-compatible`、`anthropic` 或 `gemini`；远端 endpoint 必须使用 HTTPS（loopback 可用 HTTP）。Cista 从配置名称推导内部 ID `model-secret-library/<name>`：每个新请求优先读取系统保险库，不存在时读取 daemon 当前进程中的可选 `secret_env`，但不会把环境变量导入或反向覆盖保险库。用户配置不接受模型 `secret_ref`。配置名称通过内部 `name` 字段传播；动态请求参数 `provider` 原样进入上游 HTTP body，二者不复用字段。
